...
public class BSTree1<E extends Comparable<E>> {
	...
	private Deque<Node<E>> getLargestRangePath(Node<E> node, E a, E b,
			Map<Node<E>, Node<E>> parent) {
	...
	public int getRangePathLen(Node<E> node, E a, E b,
			Map<Node<E>, Node<E>> parent) {
		if (node == null)
			return 0;
		int incr = 0;
	...
	public void getRangePath(Node<E> node, E a, E b, Deque<Node<E>> deck,
			Map<Node<E>, Node<E>> parent) {
		if (node == null)
			return;
		if (node.value.compareTo(a) >= 0 && node.value.compareTo(b) <= 0) {
