...
public class Mergesort1 {
	public static void mergesortWithArrays(String debug, Integer[] d, int low,
			int high) {
		// if we have a slice of 1 element or an empty slice return (1 element
		// is sorted!)
		int mid;
		System.out.println(debug + "/" + low + "/" + high + "/recur:"
				+ Arrays.toString(d));
		if (high - low > 1) {
			mid = (low + high) / 2;
	...
	public static <E extends Comparable<E>> void mergeArrays(E[] d, int low,
			int mid, int high) {
		...
		for (E e : buf2)
			d[x++] = e;
		System.out.println(low + "/" + high + "/(" + mid + ")/result"
				+ Arrays.toString(d));
	}
	...
	public static <E extends Comparable<E>> void mergeSubArrays(E[] m, E[] c1,
			E[] c2) {
		System.out.println("merge/" + Arrays.toString(c1) + "/"
				+ Arrays.toString(c2));
		int mi = 0;
		int j = 0;
		int k = 0;
