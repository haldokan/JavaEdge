...
public class Tester {
	...
	public void testCarPassengerCount() {
		...
		Set<ConstraintViolation<Vehicle>> constraintViolations = validator
				.validate(car, OverallChecks.class);
		...
		for (ConstraintViolation<Vehicle> cv : constraintViolations) {
			System.out.println("msg = " + cv.getMessage() + ", invalidValue = "
					+ cv.getInvalidValue());
			Set<Class<? extends Payload>> payloads = cv
	...
	public void testTruckPassengerCount() {
		...
		constraintViolations = validator.validate(truck,
				ValidationGroups.CapacityChecks.class);
		assertEquals(0, constraintViolations.size());
	}
	...
	public void testTbd() {
		...
		constraintViolations = validator.validate(car,
				ValidationGroups.CarChecks.class);
		...
		car.setDriver(john);
		constraintViolations = validator.validate(car,
				ValidationGroups.DriverChecks.class);
		assertEquals(1, constraintViolations.size());
		...
		assertEquals(0,
				validator.validate(car, ValidationGroups.DriverChecks.class)
						.size());

		// just checking that everything is in order now
		assertEquals(
				0,
				validator.validate(car, Default.class,
						ValidationGroups.CarChecks.class,
