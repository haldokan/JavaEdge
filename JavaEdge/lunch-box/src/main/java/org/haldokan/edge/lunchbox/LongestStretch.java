...
public class LongestStretch {
	public static void main(String[] args) {
		...
		Integer[] a = new Integer[] { 0, 0, 1, 1, 1, 0, 0, 0, 1, 1, 0, 0, 1, 1,
		...
		for (int i = 1; i < a.length; i++) {
			...
			if (a[i] == prev) {
				...
				maxStretch.compute(prev, (k, v) -> (v == null) ? prevStretch
						: (prevStretch.len > v.len) ? prevStretch : v);
				cn = 1;
				Stretch currStretch = new Stretch(i, i + 1, cn);
				maxStretch.compute(a[i], (k, v) -> (v == null) ? currStretch
						: (currStretch.len > v.len) ? currStretch : v);
				prev = a[i];
