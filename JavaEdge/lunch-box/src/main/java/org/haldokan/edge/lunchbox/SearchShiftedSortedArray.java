...
public class SearchShiftedSortedArray {
	...
	int ascIndex = findInfectionPoint(arr);
	if (ascIndex == -1 || elem <= arr[arr.length - 1])
	    return binsearch(arr, elem, ascIndex, arr.length);
	else
	    return binsearch(arr, elem, 0, ascIndex);
    }
