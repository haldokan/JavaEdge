...
public class UpdatesSpout {
	...
	public UpdatesSpout(BlockingQueue<Update> updateQueue,
			TrendDetector trendDetector, int concurrLevel) {
			...
			public void run() {
				for (;;) {
					try {
						...
						if (sw == null) {
							sw = new SlidingTimeWindow(windowLength,
									windowShift, trendDetector);
