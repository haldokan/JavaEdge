...
public class ConcurrentHashMap2Test {
	...
	ConcurrentHashMap2<String, Integer> m = new ConcurrentHashMap2<>(4, (String k) -> k.length(), 1);
	String s = "foobar";
	for (int i = 0; i < 10; i++) {
	    m.put(s, i * s.length());
	...
	// we are passing a hash function that takes the prize for dumbness
	ConcurrentHashMap2<String, Integer> m = new ConcurrentHashMap2<>(10, (String k) -> k.length(), 1);
	String s1 = "foobar";
	m.put(s1, s1.length());
	...
	// we are passing a hash function that takes the prize for dumbness
	ConcurrentHashMap2<String, Integer> m = new ConcurrentHashMap2<>(1024, (String k) -> k.length(), 1);
	String key = "key";
	int sampleSize = 512;
	...
	ConcurrentHashMap2<String, Integer> m = new ConcurrentHashMap2<>(spaceSize, k -> new BitwiseHasher().apply(k),
		4);
	String key1 = "key0001";
