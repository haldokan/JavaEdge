...
public class ExecutorService2Test {
	...
	private static List<String> states = Arrays
			.asList(new String[] { "NewYork", "NewJersey", "Maine",
					"Pennsylvania", "Florida", "NewHampshire", "Connecticut",
					"California", "Texas", "Washignton" });

	@Test
	public void testSubmit() throws Exception {
		int poolSize = 3;
		FixedThreadPoolExecutorService<String> es = new FixedThreadPoolExecutorService<>(
				poolSize);

		List<Future<String>> futureResults = new ArrayList<>();

		for (String state : states) {
			futureResults.add(es.submit(getCallable(state,
					100 + rand.nextInt(50))));
		}
	...
	public void testSubmitWithLargeNumberOfThreads() throws Exception {
		...
		for (int i = 0; i < inputSize; i++) {
			input.add(states.get(rand.nextInt(states.size()))
					+ rand.nextInt(5000));
		}
