...
public class PhoneBookForThePlanetPopulation {
    /**
     * 7 billion people with: Lets assume that a person name can fit in 30 chars which require 30 * 2 = 60 bytes per
     ...
     * - Create metadata files that has info on which files has which key ranges
     ...
     * - Instead of using the master server to route requests we can use a client library that downloads the metadata files to the client
     */
}
